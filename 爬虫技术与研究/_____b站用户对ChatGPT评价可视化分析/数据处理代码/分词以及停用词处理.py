...
if __name__ == '__main__':  # 判断是否为主程序
    ...
    with open(r"datas\弹幕数据\自己收集的弹幕文件.txt", "r", encoding="utf-8") as f:  # 以只读模式打开弹幕数据文件
    ...
    with open(r"datas\处理后\分词以及停用词处理后弹幕数据.txt", "w", encoding="utf-8") as fh:  # 以写入模式打开处理后的弹幕数据文件
